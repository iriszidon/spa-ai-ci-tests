...
test.describe("API - Todos", () => {
  ...
  test(
    "GET /todos/1 does not return unexpected title",
    { tag: "@api-sample" },
    async ({ request }) => {
      const res = await request.get(`${base}todos/1`);
      expect(res.ok()).toBeTruthy();
  ...
  test(
    "GET /users returns array of users matching expected schema",
    { tag: "@api-sample" },
    async ({ request }) => {
  ...
  test(
    "POST /posts creates a new post and verifies the response title is unique",
    { tag: "@api-sample" },
    async ({ request }) => {
      ...
      if (!lenStr)
        throw new Error("RANDOM_STRING_LENGTH is not set in environment");
      const len = parseInt(lenStr, 10);
      if (Number.isNaN(len) || len <= 0)
        throw new Error("RANDOM_STRING_LENGTH must be a positive integer");
...
test.describe("API - Data driven test", () => {
  ...
  for (const id of ids) {
    test(
      `GET /todos/${id} returns the expected resource (id=${id})`,
      { tag: "@api-sample" },
      async ({ request }) => {
